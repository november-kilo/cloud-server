...
static object compile_object(string path, string source...)
{
	...
	error("Access denied in compile_object()");
    }
...
static object clone_object(string path, varargs string uid)
{
	 ...
	 */
	error("Access denied in clone_object()");
    }
	...
	if (sscanf(str, "/kernel/%*s") != 0 ||
	    (creator != "System" &&
	     !::find_object(ACCESSD)->access(object_name(this_object()), str,
					     READ_ACCESS))) {
	    error("Access denied in new_object()");
	}
...
static string read_file(string path, varargs int offset, int size)
{
	...
	!::find_object(ACCESSD)->access(object_name(this_object()), path,
					READ_ACCESS)) {
	error("Access denied in read_file()");
    }
...
static int write_file(string path, string str, varargs int offset)
{
	...
	(creator != "System" &&
	 !::find_object(ACCESSD)->access(object_name(this_object()), path,
					 WRITE_ACCESS))) {
	error("Access denied in write_file()");
    }
...
static int remove_file(string path)
{
	...
	(creator != "System" &&
	 !::find_object(ACCESSD)->access(object_name(this_object()), path,
					 WRITE_ACCESS))) {
	error("Access denied in remove_file()");
    }
...
static mixed **get_dir(string path)
{
	...
	!::find_object(ACCESSD)->access(object_name(this_object()), path,
					READ_ACCESS)) {
	error("Access denied in get_dir()");
    }
...
static mixed *file_info(string path)
{
	...
	!::find_object(ACCESSD)->access(object_name(this_object()), path,
					READ_ACCESS)) {
	error("Access denied in file_info()");
    }
...
static int make_dir(string path)
{
	...
	(creator != "System" &&
	 !::find_object(ACCESSD)->access(object_name(this_object()), path,
					 WRITE_ACCESS))) {
	error("Access denied in make_dir()");
    }
...
static int remove_dir(string path)
{
	...
	(creator != "System" &&
	 !::find_object(ACCESSD)->access(object_name(this_object()), path,
					 WRITE_ACCESS))) {
	error("Access denied in remove_dir()");
    }
...
static int restore_object(string path)
{
    ...
    if (creator != "System" && !::find_object(ACCESSD)->access(oname, path, READ_ACCESS)) {
        error("Access denied in restore_object()");
    }
...
static void save_object(string path)
{
	...
	error("Access denied in save_object()");
    }
